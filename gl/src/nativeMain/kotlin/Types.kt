...
actual inline fun GL11.makeCurrent() {}
...
actual class GLDebugMessageCallback @PublishedApi internal constructor(
    handler: GLDebugMessageCallbackHandler
) {
    @PublishedApi
    internal var ref: StableRef<GLDebugMessageCallbackHandler>? =
        StableRef.create(handler)
...
interface GLDebugMessageCallbackHandler {
    operator fun invoke(
...
) = GLDebugMessageCallback(object : GLDebugMessageCallbackHandler {
    override fun invoke(
        source: GLenum, type: GLenum, id: GLuint, severity: GLenum,
        message: String?
...
@PublishedApi
internal fun glDebugMessageCallbackStatic(
    source: UInt, type: UInt, id: UInt, severity: UInt, length: Int,
    message: CPointer<ByteVar>?, userParam: COpaquePointer?
) {
    val callback = userParam!!.asStableRef<GLDebugMessageCallbackHandler>()
    val messageString = message?.readBytes(length)?.stringFromUtf8()
    callback.get()(
        source.toInt(), type.toInt(), id, severity.toInt(), messageString
    )
}
