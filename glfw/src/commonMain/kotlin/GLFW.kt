...
expect fun glfwInitHint(
    hint: Int, value: Int
)

expect fun glfwGetVersion(
    major: IntArray?, minor: IntArray?, rev: IntArray?
)

expect fun glfwGetVersionString(
): String

expect fun glfwGetError(
    description: Array<String?>?
): Int

expect fun glfwSetErrorCallback(
    cbfun: GLFWErrorCallback?
): GLFWErrorCallback?

expect fun glfwGetMonitors(
): GLFWMonitorBuffer?

expect fun glfwGetPrimaryMonitor(
): GLFWMonitor
...
expect fun glfwGetMonitorName(
    monitor: GLFWMonitor
): String?

expect fun glfwSetMonitorUserPointer(
    monitor: GLFWMonitor, pointer: Long
)

expect fun glfwGetMonitorUserPointer(
    monitor: GLFWMonitor
): Long
...
expect fun glfwGetVideoModes(
    monitor: GLFWMonitor
): GLFWVidModeBuffer?

expect fun glfwGetVideoMode(
    monitor: GLFWMonitor
): GLFWVidMode?

expect fun glfwSetGamma(
    monitor: GLFWMonitor, gamma: Float
)

expect fun glfwGetGammaRamp(
    monitor: GLFWMonitor
): GLFWGammaRamp?

expect fun glfwSetGammaRamp(
...
expect fun glfwWindowHint(
    hint: Int, value: Int
)

expect fun glfwWindowHintString(
...
expect fun glfwSetWindowAspectRatio(
    window: GLFWWindow, numer: Int, denom: Int
)

expect fun glfwSetWindowSize(
...
expect fun glfwGetWindowOpacity(
    window: GLFWWindow
): Float

expect fun glfwSetWindowOpacity(
    window: GLFWWindow, opacity: Float
)

expect fun glfwIconifyWindow(
    window: GLFWWindow
)

expect fun glfwRestoreWindow(
    window: GLFWWindow
)

expect fun glfwMaximizeWindow(
    window: GLFWWindow
)

expect fun glfwShowWindow(
    window: GLFWWindow
)

expect fun glfwHideWindow(
    window: GLFWWindow
)

expect fun glfwFocusWindow(
    window: GLFWWindow
)

expect fun glfwRequestWindowAttention(
    window: GLFWWindow
)

expect fun glfwGetWindowMonitor(
    window: GLFWWindow
): GLFWMonitor
...
expect fun glfwGetWindowAttrib(
    window: GLFWWindow, attrib: Int
): Int

expect fun glfwSetWindowAttrib(
    window: GLFWWindow, attrib: Int, value: Int
)

expect fun glfwSetWindowUserPointer(
    window: GLFWWindow, pointer: Long
)

expect fun glfwGetWindowUserPointer(
    window: GLFWWindow
): Long
...
expect fun glfwWaitEventsTimeout(
    timeout: Double
)
...
expect fun glfwGetInputMode(
    window: GLFWWindow, mode: Int
): Int

expect fun glfwSetInputMode(
    window: GLFWWindow, mode: Int, value: Int
)

expect fun glfwGetKeyName(
    key: Int, scancode: Int
): String?

expect fun glfwGetKeyScancode(
    key: Int
): Int

expect fun glfwGetKey(
    window: GLFWWindow, key: Int
): Int

expect fun glfwGetMouseButton(
...
expect fun glfwSetCursorPos(
    window: GLFWWindow, xpos: Double, ypos: Double
)

expect fun glfwCreateCursor(
    image: GLFWImage, xhot: Int, yhot: Int
): GLFWCursor

expect fun glfwCreateStandardCursor(
    shape: Int
): GLFWCursor

expect fun glfwDestroyCursor(
    cursor: GLFWCursor
)

expect fun glfwSetCursor(
...
expect fun glfwJoystickPresent(
    jid: Int
): Boolean

expect fun glfwGetJoystickAxes(
    jid: Int
): FloatReadBuffer?

expect fun glfwGetJoystickButtons(
    jid: Int
): ByteReadBuffer?

expect fun glfwGetJoystickHats(
    jid: Int
): ByteReadBuffer?

expect fun glfwGetJoystickName(
    jid: Int
): String?

expect fun glfwGetJoystickGUID(
    jid: Int
): String?

expect fun glfwSetJoystickUserPointer(
    jid: Int, pointer: Long
)

expect fun glfwGetJoystickUserPointer(
    jid: Int
): Long

expect fun glfwJoystickIsGamepad(
    jid: Int
): Boolean

expect fun glfwSetJoystickCallback(
    cbfun: GLFWJoystickCallback?
): GLFWJoystickCallback?

expect fun glfwUpdateGamepadMappings(
    string: String
): Boolean

expect fun glfwGetGamepadName(
    jid: Int
): String?

expect fun glfwGetGamepadState(
    jid: Int, state: GLFWGamepadState
): Boolean

expect fun glfwSetClipboardString(
    window: GLFWWindow, string: String
)

expect fun glfwGetClipboardString(
    window: GLFWWindow
): String?

expect fun glfwGetTime(
): Double

expect fun glfwSetTime(
    time: Double
)

expect fun glfwGetTimerValue(
): Long

expect fun glfwGetTimerFrequency(
): Long

expect fun glfwMakeContextCurrent(
    window: GLFWWindow
)

expect fun glfwGetCurrentContext(
): GLFWWindow

expect fun glfwSwapBuffers(
    window: GLFWWindow
)

expect fun glfwSwapInterval(
    interval: Int
)
